...
def calculate_distance(row):
    """
    Calculate the geodesic distance in KMs between two geographic points.

    The points are given by latitude and longitude values in the row:
    - ('lat', 'long') represent the first point coordinates.
    - ('merch_lat', 'merch_long') represent the second point coordinates.

    Parameters:
        row (pandas.Series): A row from a DataFrame containing the coordinates.

    Returns:
        float: The distance in kilometers between the two points.
               Returns 0.0 if the calculation fails.
    """
    try:
        return geodesic(
        ...
        logger.warning("Geo distance calculation failed for row: %s", e)
        return 0.0
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        logger.error("Usage: python -m src.features.features <config.yaml>")
        sys.exit(1)
