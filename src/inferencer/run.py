...
def main(cfg: DictConfig):
    ...
    try:
        ...
        if not inference_data_path.exists():
            logger.error(f"Inference data not found at {inference_data_path}")
            sys.exit(1)
        ...
        input_features_raw = [f for f in raw_features if f != target and f in df.columns]
        ...
        # The preprocessor here is the full pipeline (imputation, scaling, encoding)
        X_processed = preprocessor.transform(X_raw)

        predictions = model.predict(X_processed)
        predict_proba = model.predict_proba(X_processed)[:, 1] if hasattr(model, "predict_proba") else None
        ...
        predictions_art = wandb.Artifact(
            ...
            type="predictions",
            description="Predictions from the inference pipeline on unseen data.",
        )
